...
class ImageProcessor:
    # class provides methods to manipulate images including loading, storing, 

    @staticmethod
    def from_np_to_pil(
        img_np: np.ndarray, assume_float01: bool = True, make_rgb: bool = False
    ) -> Image.Image:
        """
        Converts a NumPy image (H,W[,C]) → PIL.Image.Image.
    ...
    @staticmethod
    def downscale_to_dir(
        inputFolder, outputFolder, new_size, interpolation=cv2.INTER_AREA
    ):
        """
    ...
    def rotate_to_dir(inputFolder, outputFolder, rotAngle):
        ...
        print(
            f"rotating {len(imgs)} images by {angleStr[rotAngle]}\nfrom {inputFolder} \nto {outputFolder}"
        )
    ...
    @staticmethod
    def copyRenameDownscaleRotateSave(
        imgs, imgNames, to_dir, new_resolution, strip_prefix, add_prefix, rotAngle
    ):
    ...
    @staticmethod
    def downscaleToFolder(
        inputFolder, outputFolder, new_size, interpolation=cv2.INTER_AREA, debug=True
    ):
        # overwrites already existing files if the name is equal
        # get image names
        filenames = ImageProcessor.list_files(
            inputFolder
        )  # Returns a list of filenames

        # load images
        imgs, iNames = ImageProcessor.load_images(
            imgNames=filenames, from_dir=inputFolder
        )
        ...
        for img, name in zip(imgs, iNames):
            ...
            try:
                dImg = ImageProcessor.downscale(
                    img, new_size, interpolation, plotResult=False
                )
    ...
    def getRoiWithResizedMask(img, mask, contourThiknes=1, plotResult=True):
        ...
        cv2.drawContours(
            overlay_image, contours, -1, (0, 0, 255), thickness=contourThiknes
        )
    ...
    def class_distribution(image_directories):
        class_counts = {
            category: len(os.listdir(paths["images"]))
            for category, paths in image_directories.items()
        }
    ...
    def get_image_metadata(image_directory):
        ...
        all_images = [
            img for img in os.listdir(image_directory) if img.endswith(".png")
        ]
    ...
    def generate_masked_images(
        ...
        imgs, names = ImageProcessor.load_images(
